...
function hideAdsByKeywords() {
    console.log("Début du filtrage des annonces sur Jooble...");

    chrome.storage.local.get(["keywords"], (result) => {
        const userKeywords = result.keywords || [];
        const allKeywords = keywords.concat(userKeywords);

        const relevantElements = document.querySelectorAll('li div[data-test-name="_jobCard"]');
        ...
        for (let element of relevantElements) {
            ...
            const companyElement = element.querySelector('p[data-test-name="_companyName"]');
...
chrome.runtime.onMessage.addListener((request) => {
    if (request.action === "updateKeywords") {
        console.log("Mise à jour des mots-clés reçue pour Jooble.");
        hideAdsByKeywords();
    }
});
